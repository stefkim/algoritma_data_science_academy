...
shinyServer(function(input, output) {
   ...
   output$plot_earn <- renderPlotly({
      
      workers_agg1 <- workers_clean %>% 
         filter(year == input$year_id) %>% # ambil data tahun 2016
         mutate(gap_earnings = total_earnings_male - total_earnings_female) %>%  # buat kolom baru gap_earnings
         group_by(major_category, year) %>%  # mengelompokkan data berdasarkan major category
      ...
      plot_earnings <-  ggplot(data = workers_agg1, mapping = aes(y = reorder(major_category, avg_earnings), x = avg_earnings)) +
         ...
         labs(title = paste("Average Gap Earnings in",input$year_id), 
              y = "Major Category", 
              x = NULL) +
         theme_minimal() +
