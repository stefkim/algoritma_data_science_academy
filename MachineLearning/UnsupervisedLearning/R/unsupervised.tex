...
\NormalTok{whis.pca<-}\KeywordTok{PCA}\NormalTok{(whi_z, }\DataTypeTok{graph =}\NormalTok{ F)}
\KeywordTok{plot}\NormalTok{(whis.pca, }\DataTypeTok{choix=}\KeywordTok{c}\NormalTok{(}\StringTok{"ind"}\NormalTok{), }\DataTypeTok{label=}\StringTok{"none"}\NormalTok{, }\DataTypeTok{col.ind=}\NormalTok{whiskies}\OperatorTok{$}\NormalTok{clust)}
\KeywordTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\KeywordTok{levels}\NormalTok{(whiskies}\OperatorTok{$}\NormalTok{clust), }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{, }\DataTypeTok{col=}\DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# get the PC1 and PC2 coordinates for laphroig}
\NormalTok{indx <-}\StringTok{ }\KeywordTok{which}\NormalTok{(}\KeywordTok{rownames}\NormalTok{(whiskies) }\OperatorTok{==}\StringTok{ "Laphroig"}\NormalTok{)}
\NormalTok{laph <-}\StringTok{ }\NormalTok{whis.pca}\OperatorTok{$}\NormalTok{ind}\OperatorTok{$}\NormalTok{coord[indx,}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{]}

\CommentTok{# creates individuals factor map}
\KeywordTok{plot}\NormalTok{(whis.pca, }\DataTypeTok{choix=}\KeywordTok{c}\NormalTok{(}\StringTok{"ind"}\NormalTok{), }\DataTypeTok{label=}\StringTok{"none"}\NormalTok{, }\DataTypeTok{col.ind=}\NormalTok{whiskies}\OperatorTok{$}\NormalTok{clust)}

\CommentTok{# add points and text}
\KeywordTok{points}\NormalTok{(laph[}\DecValTok{1}\NormalTok{], laph[}\DecValTok{2}\NormalTok{], }\DataTypeTok{col=}\StringTok{"dodgerblue4"}\NormalTok{, }\DataTypeTok{cex=}\FloatTok{1.2}\NormalTok{)}
\KeywordTok{text}\NormalTok{(laph[}\DecValTok{1}\NormalTok{], laph[}\DecValTok{2}\NormalTok{]}\OperatorTok{-}\FloatTok{0.3}\NormalTok{, }\DataTypeTok{col=}\StringTok{"dodgerblue4"}\NormalTok{, }\DataTypeTok{labels =} \StringTok{"Laphroig"}\NormalTok{, }\DataTypeTok{cex=}\FloatTok{0.6}\NormalTok{)}
\KeywordTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\KeywordTok{levels}\NormalTok{(whiskies}\OperatorTok{$}\NormalTok{clust), }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{, }\DataTypeTok{col=}\DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{unsupervised_files/figure-latex/unnamed-chunk-58-1.pdf}
