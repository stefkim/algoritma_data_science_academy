---
title: "Algoritma Academy: Classification 2"
author: "Samuel Chan"
...
To get an intuition of how this work, consider the numerator: it's computing the probability of a spam and the word "lottery" happening at the same time. We then discount this probability for the occurence of the word "lottery" in general, including the instances where it appears in a genuine email.
...
Suppose we have a partition of data with imported (70%) and local (30%), the entropy is:
p being 0.7 / 0.3
```{r}
...
```{r}
diabetes <- read.csv("data_input/diabetes.csv", stringsAsFactors = TRUE)
head(diabetes)
```
...
I'm going to create our Random Forest now, using a 5-Fold Cross Validation, with 3 repeats. A 10-Fold with 5 repeats will look like this and is not the same as a 50-Fold:
![](assets/kfold.png)
